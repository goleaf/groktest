...
function actionFor(loan: Loan, today: CalendarDate, repayments: readonly Repayment[], locale: string): HomeAction {
...
export function summarizeHome(
    ...
    const ranked = [...active].sort((left, right) => urgencyRank(left, today) - urgencyRank(right, today));
    const actions = ranked.slice(0, 8).map((loan) =>
        actionFor(loan, today, repaymentsByLoan.get(loan.id) ?? [], locale),
    );
    return {
